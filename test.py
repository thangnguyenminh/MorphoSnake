...
def test_starfish():
    # Load the image.
    imgcolor = imread("testimages/1-8.png")/255.0
    img = rgb2gray(imgcolor)
    
    # g(I)
    gI = morphsnakes.gborders(img, alpha=3000, sigma=2)
    
    # Morphological GAC. Initialization of the level-set.
    mgac = morphsnakes.MorphGAC(gI, smoothing=100, threshold=0.9, balloon=-1)
    mgac.levelset = circle_levelset(img.shape, (1750, 1000), 500, scalerow=0.75)
